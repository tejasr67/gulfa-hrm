...
import { bulkAccommodationOps } from "@/modules/accommodation/bulk";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (rows.length > 300) return err("Max 300 rows per import");
    const results = await bulkAccommodationOps(session.companyId, rows, session.userId);
